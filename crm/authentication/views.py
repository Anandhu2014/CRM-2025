...
class OTPView(View):
    ...
    def get(self,request,*args,**kwargs):
        ...
        email_otp,phone_otp = generate_otps()  #unpacking
        ...
        #for making otp expiry
        request.session['otp_time'] = timezone.now().timestamp()  #session used to store the data.after when submitting it checks the time in session.if it exceeds the remaining time then consider it expired
...
class ChangePasswordView(View):
    ...
    def post(self,request,*args,**kwargs):
        ...
        if form.is_valid():
            ...
            messages.success(request,'Password Uploaded Successfully')
