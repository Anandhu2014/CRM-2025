...
def sent_email(recepient,template,title,context):
    ...
    context =render_to_string(template,context)    #EmailMultiAlternatives takes only text values .so for making html in text format we use render_to_string
    
    msg = EmailMultiAlternatives(from_email=sender,to=[recepient],subject=title)
    
    msg.attach_alternative(context,'text/html') # text/html is mimie type.as same we used mime type as encrypte =mutipart/... in html form
